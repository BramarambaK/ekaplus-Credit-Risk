 ...
 *
 */
public class PBSCreditCheckCalculator extends CreditCheckCalculator {
	...
	public PBSCreditCheckCalculator(
	...
	public void deriveFinalResponse() {
		...
		if (this.finalAvailableBalance.compareTo(BIG_DECIMAL_ZERO) >= 0) {
			...
			itemResponse.setDescription(String.format(CreditRiskConstants.COUNTERPARTY_CREDIT_CHECK_SUCCESS,
					this.getCounterpartyGroup()!=null?this.getCounterpartyGroup():counterparty));
			...
			if (this.getCounterpartyGroup() != null) {
				if (derivedPaymentTerms.get(paymentTerm).equalsIgnoreCase(
					...
					if(this.getEventName()!=null && this.getEventName().toUpperCase().contains("LINKAGE")){
					itemResponse
							.setDescription(String
									.format(CreditRiskConstants.COUNTERPARTYGROUP_PBS_MO_MESSAGE,
											this.getCounterpartyGroup(),
